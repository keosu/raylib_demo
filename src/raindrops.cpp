...
uniform_real_distribution<float> dist1(-248, 248);
// Drop placement. Decrease range from 250 by max drop radius (in dist2 below) so drops don't overhang glass
uniform_real_distribution<float> dist2(0.7, 1.3);
// Radius range of new drops.  Greater than 1.68 starts drop in motion (for vel=mass/30)
// The maximum number of drops on screen depends on the radius range above.
// A smaller lower bound for dist2 results in more "fine mist" and thus many more particles.  Keeps 30 fps till around
// 11K drops.
...
class raindrop {
 public:
  ...
  raindrop() {
    ...
    velocity = {
        0.0f, (mass < 10.0) ? 0.0f : mass / -30.0f,
        0.0f};  // velocity is a function of mass in this sim.  Keep value of mass/-30 matched to similar line below.
    coll_flag = 0;
  }
  ...
  // So that drops which might collide are near each other in this list.  Reduces collision detection from O(n^2) to
  // O(kn) time complexity.
  bool operator<(const raindrop& a) const { return (position.x + radius) < (a.position.x - a.radius); }
};
...
class dropcontroller {
 public:
  ...
  dropcontroller() { drops.push_back(raindrop()); }
  ...
  void move_drops() {
    ...
    for (auto& drop : drops) {
      if (drop.velocity.y != 0.0) {  // If velocity = 0, we exit immediately
        ...
        // New drops are spawned at current drop location, then drop is advanced by vel pixels, so vel must be greater
        // than radius of drop Third condition = Don't bother merging if we are very close to bottom of glass.
        if ((drop.velocity.y < -3.0) && (abs(drop.velocity.y) > (1.1 * (drop.radius))) && (drop.position.y > -240.0)) {
          raindrop newdrop;
          newdrop.position = drop.position;
          ...
          drop.mass -=
              newdrop.mass;  // We decrease the mass and radius of the drop to account for part of it breaking off
          drop.radius = cbrt(6.0 * drop.mass / (4.0 * PI));
          drop.movedrop();
        } else {
          drop.movedrop();  // If you don't want streaks, this is the only line you need to move the drops
        }
      }
    }
  ...
  void coll_check() {
    ...
    for (unsigned i = 0; i < dsize - 2; i++) {
      for (unsigned j = i + 1; j < dsize - 1; j++) {
        // The magic of sorting: Because the list is sorted, once the right-hand side of drop[i] is less than the
        // left-hand side of drop[j], we've detected possible collisions, since the list is sorted by left & right x
        // coordinate boundaries.  So then we break out of inner for-loop.  Not perfect, but very good for the speed
        // improvement.
        if (drops[i].position.x + drops[i].radius < drops[j].position.x - drops[j].radius) {
          break;
        }
        ...
        ra = drops[i].radius;
        rb = drops[j].radius;
        if ((dx + dy) < ((ra + rb) * (ra + rb))) {  // If they have collided, assigned both particles a coll_flag group
                                                    // number.  Helps when more than two drops collide.
          CFnum++;
    ...
    for (int c = 1; c <= CFnum; c++) {
      // Scan through and gather each collision-flagged particle by its collflag group number
      coalesced.clear();
      for (auto& drop : drops) {
        if (drop.coll_flag == c) {
          coalesced.insert(coalesced.end(), drop);
        }
      }
      counter = 0;
      temp_mass = 0.0;
      temp_pos = {0.0, 0.0, 0.0};
      for (vector<raindrop>::iterator it2 = coalesced.begin(); it2 != coalesced.end();
           it2++) {  // Coalesce everything with the same collflag into one new drop
        temp_pos = Vector3Add(temp_pos, it2->position);
        counter++;
        temp_mass += it2->mass;
      }
      // Crude centroid, but doesn't currently take into account the varying masses of drops.
      temp_pos = Vector3Scale(temp_pos, 1.0 / ((float)counter));
      newdrop.position = temp_pos;  // Nonetheless, looks good because surface tension tends to pull real drops around
                                    // when they coalesce.
    ...
    // Having coalesced all the drops and added their merged form to newdrops, do one final pass of the list to erase
    // all coll-flagged items
    auto Cull = [](const raindrop& p) { return (p.coll_flag != 0); };
  ...
  void cull_offscreen() {
    // Remove drop from system once it falls 50px below glass pane and so is off screen
    auto Offscreen = [](const raindrop& p) { return (p.position.y < -300); };
...
Vector3 shaper(raindrop r) {
  ...
  if (r.velocity.y > -0.1) {
    return Vector3Scale(scale, r.radius);
  }
  if (r.velocity.y < -9.9) {
    return Vector3Scale(scale, r.radius);
  }                                        // If vel<-10, drop is probably in free-fall, so resume spherical shape.
...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    for (const auto& drop : my_dc.drops) {
      // DrawModel(droplet,drop.position,drop.radius,WHITE);  //If you don't want drop shaping as a function of speed,
      // use this line and comment out the one below.  May help performance a little.
      DrawModelEx(droplet, drop.position, {1.0, 0.0, 0.0}, 0.0, shaper(drop), WHITE);
    }
    EndMode3D();

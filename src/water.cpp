...
struct sphere {
  Vector3 location = {100.0, 0.0, 100.0};
...
Color colorizer(matter* matArray, int xsize, int zsize, int posx, int posz, Vector3 eye) {
  ...
  Vector3 blueparts = {blue.r, blue.g, blue.b};
  Vector3 lightparts = {light.r, light.g, light.b};
  Vector3 faderesults = {0, 0, 0};
  Vector3 pos = {posx, matArray[posx + posz * xsize].height, posz};
  Vector3 lightsource = {0.0, 100.0, 0.0};
  ...
  if ((posx == 0) || (posz == 0) || (posx >= xsize - 1) ||
      (posz >= zsize - 1)) {  // Being lazy, not calculating reflection for voxels at border with a special routine
    return {0, 121, 241, 128};
  }

  fade = Vector3DotProduct(
      Vector3Reflect(Vector3Normalize(Vector3Subtract(pos, eye)), matArray[posx + posz * xsize].normal),
      Vector3Normalize(l_to_p));
...
void calc_gradient_and_normal(matter* matArray, int xsize, int zsize) {
  ...
  for (int i = 1; i < xsize - 1; i++) {
    for (int j = 1; j < zsize - 1; j++) {
      ...
      matArray[i + j * xsize].gradient = {dx, 0.0, dz};
      matArray[i + j * xsize].normal = Vector3Normalize(Vector3CrossProduct({0.0, dz, 1.0}, {1.0, dx, 0.0}));
    }
  }
  return;
}
...
void update_spheres(sphere* spheres, matter* matArray, int xsize, int zsize, int num_spheres) {
  ...
  Vector3 temploc = {0.0, 0.0, 0.0}, tempvel = {0.0, 0.0, 0.0};
  for (int s = 0; s < num_spheres; s++) {
    ...
    // volume of sphere underwater if it is z of 2r submerged: pi*(-z^3/3+r*z^2) --> vol * mass of water displaced =
    // bouyant force upward by Archimedes' principle
    if (depth < 0) {                 // ball is higher than water, so falls straight down
      ...
      if (depth > 2.0) {
        depth = 2.0;
      }  // presuming sphere of radius one, if depth > 2.0, it is wholly submerged
      subvol = PI * (-1.0 * depth * depth * depth / 3.0 + 1.0 * depth * depth);  // 1.0 factor=radius of sphere;
      spheres[s].velocity.x -=
          5.0 * matArray[xpos + zpos * xsize].gradient.x;  // 5.0 arbitrary scale factor till it "looks right"
      spheres[s].velocity.z -= 5.0 * matArray[xpos + zpos * xsize].gradient.z;
      spheres[s].velocity.y +=
          subvol - 0.9;  // taking density of water here as 1.0, -0.9 because gravity still weighing it down too;
      spheres[s].velocity.y +=
          matArray[xpos + zpos * xsize].vel;  // if submerged, water velocity upward will also transfer to sphere
      spheres[s].velocity =
          Vector3Add(spheres[s].velocity,
                     Vector3Scale(spheres[s].velocity, -0.01 * Vector3Length(spheres[s].velocity)));  // viscosity drag
    }

    temploc =
        Vector3Add(spheres[s].location,
                   Vector3Scale(spheres[s].velocity,
                                0.03));  // temporarily assign position before checking for collision with boundary

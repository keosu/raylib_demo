...
// Our "Apollonian circle" struct defines a circle uniquely by its signed bend = 1/radius, where bend is negative if the
// circle inscribes other circles and its bend product, defend below.
struct AppCircle {
  double bend;
...
void RecurseCircles(AppCircle a, AppCircle b, AppCircle c, vector<AppCircle>& circles, int rl) {
  ...
  */
  ...
  // We only need the outer enclosing circle with negative signed bend for the first level of recursion to fill the
  // gasket Removing the IF yields the same shape, but with much more redundancy, skyrocketing memory and plummeting FPS
